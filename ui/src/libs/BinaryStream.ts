...
export class BinaryStream {
  ...
  setPosition = (position: number) => {
    ...
    throw new Error('Can not set an invalid position.');
  };

  /**
   * Decode a based64 string and load it into the binary stream.
   * @param base64 base64 encoded string containing the data.
   */
  loadFromBase64 = (base64: string) => {
    this.binaryData = decode(base64);
    this.position = 0;
  };
  ...
  readByte = () => {
    if (this.binaryData === null || this.position >= this.getSize()) {
      throw new Error('There is no further data available in the stream.');
    }
  ...
  readString = () => {
    ...
    try {
      ...
      throw new Error('Failed to read string from binary stream.');
    }
    return string;
  };
  ...
  writeByte = (byte: number) => {
    if (this.binaryData === null || this.position >= this.getSize()) {
      throw new Error(
        'There is no further space available in the stream to write data.',
      );
    }
   ...
   * @param string string to write to the stream
   */
